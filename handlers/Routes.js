export default class Routes {
  ...
  async putUsersHandler(req, res, db, wss) {
    if (req && req.headers && req.headers.authorization && (req.body.image || req.body.pass)) {
      const user = await db.getUserByToken(req.headers.authorization.replace('Bearer ', ''));
      if (user && user.id) {
        const userRes = await db.updateUser(user.id, req.body.pass || '', req.body.image || '');
        if (userRes && userRes.id) {
          if (req.body.image !== '') {
            const data = {
              msg: 'userChangeImage',
              user: {id: userRes.id, login: userRes.login, email: userRes.email, created: userRes.created, channels: userRes.channels, image: userRes.image, isOnline: userRes.isOnline},
              type: 'system'
            }
  ...
  async getMessagesHandler(req, res, db) {
    if (req && req.headers && req.headers.authorization) {
      const user = await db.getUserByToken(req.headers.authorization.replace('Bearer ', ''));
      if (user && user.id) {
        const arrIds = (req && req.query && req.query.id) ? req.query.id.split(',') : [];
  ...
  async postRegistrationHandler(req, res, db, wss) {
    if (req && req.body && req.body.login && req.body.pass && req.body.email) {
      ...
      if (user && user.id) {
        ...
        if (req.body.image) {
          createUserResult = await db.createUser(req.body.login, req.body.pass, req.body.email, req.body.image);  
        } else {
          createUserResult = await db.createUser(req.body.login, req.body.pass, req.body.email);
        }
          ...
          if (user && user.id) {
            ...
            if (token) {
              ...
              const data = {
                ...
                user: {id: user.id, login: user.login, email: user.email, created: user.created, channels: [], image: user.image, isOnline: false},
                type: 'system'
              }
              wss.wssSend(JSON.stringify(data), 'system')
              res.status(200).send(user);
      ...
      if (req.headers && req.headers.authorization) {
        ...
        if (user && user.id) {
            ...
            if (chan === -1) {
              ...
              res.status(504).send('Server error. Can not create channel ' + req.body.name);  
             ...
             const data = {
              ...
              res.status(500).send('Server error. Can not create channel ' + req.body.name);
            }
        } else {
          res.status(401).send('Bad token');
        }
